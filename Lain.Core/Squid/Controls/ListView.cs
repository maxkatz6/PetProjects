...
namespace Squid.Controls
{
    ...
    public class ListView : Control
    {
        ...
        public string GetAspectValue(object item, Column column)
        {
            ...
            var member =
                type.GetTypeInfo().DeclaredMembers.FirstOrDefault(m => m.Name == column.Aspect);
            
            if (member is FieldInfo)
            {
                return ((FieldInfo) member).GetValue(item).ToString();
            }
            if (member is PropertyInfo)
            {
                return ((PropertyInfo)member).GetValue(item, null).ToString();
            }
            if (member is MethodInfo)
            {
                return ((MethodInfo)member).Invoke(item, null).ToString();
            }
            return "not a member";
        }
    }
