...
class JoomlaSChat extends SChat {
    ...
    function getValidLoginUserData() {
        ...
        if($user->get('id') != 0) {
            ...
            $profile = $this->db->sqlQuery('SELECT * FROM '.J_PREFIX.'comprofiler WHERE user_id = '.$userData['userID'])->fetch();

            $userData['userInfo']['tim'] = self::getTIM(mb_convert_encoding( $profile['cb_1'], 'UTF-8'));
            $userData['userInfo']['gender'] = self::getGender(mb_convert_encoding( $profile['cb_2'], 'UTF-8'));

            $avatar = $profile['avatar'];
            $approved = $profile['avatarapproved'];
            $userData['userInfo']['avatar'] = ( $approved != '1' || is_null($avatar) || $avatar == ''
                ? ($userData['userInfo']['gender'] == 'f' ? 'images/user-Женский-thumb.png': 'images/user-Мужской-thumb.png')
