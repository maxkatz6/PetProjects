...
namespace Ormeli.Graphics
{
	public partial class GeometryGenerator
	{
		...
		static Dictionary<UndirectedEdge, int> subdividedEdges;
		...
		public static unsafe GeometryInfo<Vertex> CreateGeoSphere<Vertex>(float diameter = 1.0f, int tessellation = 3) where Vertex : struct, IVertex
		{
		...
		private static unsafe void FixPole<Vertex>(ref List<Vertex> vertices, int poleIndex) where Vertex : struct, IVertex
		{
			...
			for (ushort i = 0; i < indexList.Count; i += 3)
			{
				...
				newPoleVertex.TexCoord = new Vector2((vertices[*pOtherIndex0].TexCoord.X + vertices[*pOtherIndex1].TexCoord.X) * 0.5f, poleVertex.TexCoord.Y);
