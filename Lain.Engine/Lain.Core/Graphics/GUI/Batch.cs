...
namespace Lain.Graphics.GUI
{
    public class SpriteBatch
    {
        ...
        private Buffer Vb { get; set; }
        private Buffer Ib { get; set; }
        ...
        public void Draw(Texture texture, RectangleF dest, RectangleF? source = null, Color4? color = null)
        {
            ...
            var deltaX = 1f/(texture.Width);
            var deltaY = 1f/(texture.Height);
            ...
            _spriteQueue[_spriteQueueCount++] = new Sprite(texture,
                new Bitmap(
                    new RectangleF(dest.X/Config.Width*2 - 1, -(dest.Y/Config.Height*2 - 1), dest.Width/Config.Width*2,
                        -(dest.Height/Config.Height*2)), sr, color ?? Color.White));
        }

        public unsafe void End()
        {
            if (_spriteQueueCount == 0) return;
            if (UsingGS)
            {
                var p = (Bitmap*) Vb.MapBuffer().ToPointer();
                for (var i = 0; i < _spriteQueueCount; i++)
            ...
            {
                var p = (BitmapVertex*) Vb.MapBuffer().ToPointer();
                for (var i = 0; i < _spriteQueueCount; i++)
                {
        ...
        private void DrawBatchPerTexture(Texture texture, int offset, int count)
        {
            ...
            if (UsingGS) Effect.Render(_spriteQueue[offset].Tech, count, _vertexBufferPosition);
                ...
                Effect.Render(_spriteQueue[offset].Tech, count*IndicesPerSprite, _vertexBufferPosition*IndicesPerSprite);
