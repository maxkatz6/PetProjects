...
namespace TDFExample_ModelConverter.Core
{
    public partial class MainWindow : Form
    {
        ...
        public MainWindow()
        {
            ...
            {
                if (splitContainer1.Panel1.Width != 260) splitContainer1.SplitterDistance = 260;
            };
        ...
        /*    var le = new LightingEffect();
            le.InitializeFromFile("Light.fx");*/

            var m = GeometryGenerator.CreateGrid(1000, 1000,2,2).ToMesh();
            m.Material = new Material
            {
                DiffuseTexture = Texture.NullTexture, 
                NormalTexture = Texture.NullTexture,
                Diffuse = new Color4(1,1,1,1), 
                Ambient=new Color4(1,1,1,1),
                Specular = new Color4(1,1,1,1),
                Reflect = new Color4(1,1,1,1)
            };
            m.HasMaterial = true;
   //         m.Effect = le;
            ...
            directXPanel1.Loop = () =>
            {
                ...
                var p = WinAPI.GetCurPos();
                Input.SetMouseCoord(p.X - directXPanel1.Location.X, p.Y - directXPanel1.Location.Y - 47);
                toolStripStatusLabel1.Text = _fps.Value.ToString(CultureInfo.InvariantCulture) + "      " + _freeCamera.Position + "                " + Input.MouseState.Vector;
            };
        ...
        private void directXPanel1_Load(object sender, EventArgs e)
        {
